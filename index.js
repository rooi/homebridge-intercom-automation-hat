...
function IntercomPlatform(log, config, api) {
  ...
  this.bellTimeout = Number(this.config.bellTimeout) || 1000;
  this.lockTimeout = Number(this.config.lockTimeout) || 1000;
  this.voltageLowLimit = Number(this.config.voltageLowLimit) || 0.03;
  this.pythonPath = this.config.pythonPath || '/home/pi/venvs/automationhat/bin/python';

  this.name = this.config.name || 'Intercom';

  // Stable UUID seeds.
  // These preserve the UUIDs generated by the previous refactor:
  // lockName = this.name + "Lock" -> lockName + "LockAccessory"
  // doorbellName = this.name + "Bell" -> doorbellName + "BellAccessory"
  this.lockAccessoryId = `${this.name}LockLockAccessory`;
  this.doorbellAccessoryId = `${this.name}BellBellAccessory`;
...
IntercomPlatform.prototype = {
  ...
  setupLockAccessory: function() {
    ...
    const lockUuid = UUIDGen.generate(this.lockAccessoryId);

    this.log('Lock accessory UUID seed:', this.lockAccessoryId);
    this.log('Lock accessory UUID:', lockUuid);

    let accessory = this.accessories.get(lockUuid);

    if (accessory) {
      this.log('Restoring existing lock accessory:', accessory.displayName);
      accessory.displayName = lockName;
    } else {
      this.log('Creating new lock accessory:', lockName);
      accessory = new PlatformAccessory(lockName, lockUuid, hap.Categories.LOCK);
  ...
  setupDoorbellAccessory: function() {
    ...
    const doorbellUuid = UUIDGen.generate(this.doorbellAccessoryId);

    this.log('Doorbell accessory UUID seed:', this.doorbellAccessoryId);
    this.log('Doorbell accessory UUID:', doorbellUuid);

    let accessory = this.accessories.get(doorbellUuid);

    if (accessory) {
      this.log('Restoring existing doorbell accessory:', accessory.displayName);
      accessory.displayName = doorbellName;
    } else {
      this.log('Creating new doorbell accessory:', doorbellName);
      accessory = new PlatformAccessory(doorbellName, doorbellUuid, hap.Categories.VIDEO_DOORBELL);
      this.api.registerPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [accessory]);
    }
  ...
  configureLockService: function(accessory, lockName) {
    ...
    if (!lockService) {
      // Keep this subtype stable. Changing it can break HomeKit scenes/automations.
  ...
  setupMatterDevices: async function() {
    ...
    this.log('Matter lock UUID:', matterLockUuid);
    this.log('Matter bell sensor UUID:', matterBellSensorUuid);
